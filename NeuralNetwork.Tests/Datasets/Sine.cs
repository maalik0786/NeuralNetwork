...
namespace NeuralNetwork.Tests.Datasets
{
	public class Sine
	{
		public void Run()
		{
			var network = new Model(1, 15, 1, 0.01f);
			var shuffledInputs = GenerateShuffelInputs();
			...
			for (var epoch = 0; epoch < 100; epoch++)
				foreach (var data in trainData)
				{
					...
					var actual = (float) (Math.Sin(normalizedInput) + 1) / 2; //normalized between [0, 1]
					network.Train(new[] { (float) normalizedInput }, new[] { actual });
				}

			foreach (var data in testData.OrderBy(x => x).ToArray())
			{
				var normalizedInput = data / Math.PI;
				var result = network.Evaluate(new[] { (float) normalizedInput });
				var errorMargin = result[0] * 2 - 1 - Math.Sin(normalizedInput);
				Console.WriteLine("Final error (target - actual): " + errorMargin);
			}
		}

		private static double[] GenerateShuffelInputs()
		{
